...
st.markdown(
    ...
    /* ===== GRIDMIND EXECUTIVE OVERVIEW + INTELLIGENCE POLISH ===== */
    div[data-testid="stMetric"] {
        position: relative;
        overflow: hidden;
    }
    ...
    div[data-testid="stMetric"]::after {
        ...
        top: -62px;
        border-radius: 50%;
        background: rgba(70,215,255,.035);
        pointer-events: none;
    }
    ...
    @media (max-width: 760px) {
        .gm-title { font-size: 2.45rem; }
        .gm-subtitle { font-size: .92rem; }
        .gm-integrity-pill { white-space: normal; }
    }
...
def apply_chart_theme(
    ...
    fig.update_layout(
        ...
        legend=dict(
            ...
            font=dict(size=12, color="#c7d7e5"),
        ),
        hoverlabel=dict(
            bgcolor="#0b1726",
            bordercolor="rgba(142,190,225,.22)",
            font=dict(color="#eef7ff", size=12),
        ),
    )
    ...
    fig.update_xaxes(
        ...
        zeroline=False,
        showline=False,
        tickfont=dict(color="#91a8bb"),
        title_font=dict(color="#c8d8e6"),
    )
...
with overview_tab:
    ...
    timeline.update_layout(
        ...
        xaxis=dict(
            ...
            spikemode="across",
            spikesnap="cursor",
            spikecolor="rgba(238,247,255,.48)",
            spikethickness=1,
        ),
        hoverdistance=60,
        spikedistance=-1,
    ...
    with left:
        ...
        hist.update_traces(
            ...
            opacity=0.76,
            marker_line_width=0,
            hovertemplate="Residual: %{x:,.0f} MW<br>Hours: %{y}<extra></extra>",
        )
        ...
        if not filtered_predictions.empty:
            hist.add_vline(
                x=float(filtered_predictions["residual"].mean()),
                line_dash="dot",
                line_color="#ffbf69",
                line_width=1.6,
            )
    ...
    with right:
        ...
        benchmark.update_yaxes(
            categoryorder="array",
            categoryarray=["Random Forest", "Seasonal Naive"],
        )
...
with anomaly_tab:
    ...
    with chart_left:
        ...
        if filtered_anomalies.empty:
            ...
            donut.update_traces(
                ...
                sort=False,
                pull=[0.015 if value > 0 else 0 for value in severity_counts["Count"]],
                marker=dict(
                    line=dict(
                        color="rgba(6,16,29,.92)",
                        width=2,
                    )
                ),
            ...
            donut.add_annotation(
                x=0.5,
                y=0.5,
                text=f"<b>{int(severity_counts['Count'].sum())}</b><br><span style='font-size:11px;color:#8fa4b8'>EVENTS</span>",
                showarrow=False,
                font=dict(size=22, color="#eef7ff"),
                align="center",
            )
    ...
    with chart_right:
        ...
        if filtered_anomalies.empty:
            ...
            direction_counts = (
                filtered_anomalies["direction"]
                .value_counts()
            )

            spike_count = int(
                direction_counts.get("spike", 0)
            )
            drop_count = int(
                direction_counts.get("drop", 0)
            )
            total_count = spike_count + drop_count

            spike_pct = (
                100.0 * spike_count / total_count
                if total_count
                else 0.0
            )
            drop_pct = (
                100.0 * drop_count / total_count
                if total_count
                else 0.0
            )
...
.gm-direction-card {
    background:
        linear-gradient(135deg, rgba(13, 38, 58, 0.96), rgba(7, 24, 40, 0.98));
...
.gm-direction-total-label {
    margin-top: 7px;
    color: #91a9bc;
    font-size: 15px;
}

.gm-direction-dominant {
    padding: 9px 14px;
    border-radius: 999px;
...
.gm-direction-fill.spike {
    background: #ffbf69;
}

.gm-direction-fill.drop {
    background: #ff6685;
}
...
.gm-direction-divider {
    background: rgba(148, 163, 184, 0.18);
}

.gm-direction-stat-head {
    display: flex;
    align-items: center;
    gap: 9px;
    color: #9bb0c2;
...
.gm-direction-dot.spike {
    background: #ffbf69;
    box-shadow: 0 0 0 5px rgba(255,191,105,0.10);
}

.gm-direction-dot.drop {
    background: #ff6685;
    box-shadow: 0 0 0 5px rgba(255,102,133,0.10);
}

.gm-direction-stat-value {
    margin-top: 15px;
    color: #f4f8ff;
    font-size: 34px;
    line-height: 1;
...
""")
            ...
            st.html(
                f"""
<div class="gm-direction-card">
    <div class="gm-direction-summary">
        <div>
            <div class="gm-direction-kicker">
                EVENT DIRECTION
            </div>
            <div class="gm-direction-total">
                {total_count}
            </div>
            <div class="gm-direction-total-label">
                visible events
            </div>
        </div>

        <div class="gm-direction-dominant{dominant_class}">
            {dominant_direction}
        </div>
    </div>

    <div class="gm-direction-track">
        <div
            class="gm-direction-fill spike"
            style="width: {spike_pct:.2f}%"
        ></div>
        <div
            class="gm-direction-fill drop"
            style="width: {drop_pct:.2f}%"
        ></div>
    </div>

    <div class="gm-direction-grid">
        <div class="gm-direction-stat">
            <div class="gm-direction-stat-head">
                <span class="gm-direction-dot spike"></span>
                DEMAND SPIKES
            </div>
            <div class="gm-direction-stat-value">
                {spike_count}
            </div>
            <div class="gm-direction-stat-pct">
                {spike_pct:.1f}% of visible events
            </div>
        </div>

        <div class="gm-direction-divider"></div>

        <div class="gm-direction-stat">
            <div class="gm-direction-stat-head">
                <span class="gm-direction-dot drop"></span>
                DEMAND DROPS
            </div>
            <div class="gm-direction-stat-value">
                {drop_count}
...
<div class="gm-integrity-detail">q=0.99 estimated before final-test scoring</div>
    ...
    with meta2:
        validation_status = (
            "REVIEW REQUIRED" if validation_seen else "PREDICTION ONLY"
        )
        validation_detail = (
            "Validation data used during model fitting"
            if validation_seen
            else "Unseen residuals used only for threshold calibration"
        )
